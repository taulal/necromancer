...
### NEC-07c · cap-50 Sonnet autopsy root cause (Fri 25 Sep, Claude Code)

`necro.effectError` (12:18:20 UTC, séance `e2m8kG6gvuYt6cGzurrAFQ`, 50 pages, `claude-sonnet-4-5-20250929`, key present) was a **ZodError at `types`: "expected array, received string"**. So the bundle ran, the env was right, and Claude answered. Sonnet returned `propose_anatomy` input with the `types` array **JSON-encoded as a string**, and our parse rejected it.

- **Not truncation or timeout.** A local repro of the same call (same corpse, model, tool and `max_tokens: 16384`) took **93.9s** with **12,693 in / 5,937 out** tokens and `stop_reason: tool_use`. Netlify failed about 100s into the drain, far short of the ~4 min that 16k output tokens would take. The repro returned a proper array (7 types), so the stringification is intermittent.
